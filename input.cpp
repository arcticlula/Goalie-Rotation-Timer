...
esp_timer_handle_t timer_general_handles[NUM_OF_INPUTS];

esp_timer_create_args_t timer_debounce_args[NUM_OF_INPUTS];
esp_timer_create_args_t timer_general_args[NUM_OF_INPUTS];
...
void init_inputs(void) {
  for (int i = 0; i < NUM_OF_INPUTS; i++) {
    ...
    timer_general_args[i] = (esp_timer_create_args_t){
      .callback = &timer_general_handler,
      .arg = &input_indices[i],
      .dispatch_method = ESP_TIMER_TASK,
      .name = "timer general"
    };
    ...
    esp_timer_create(&timer_general_args[i], &timer_general_handles[i]);
  }
...
void add_click_handler(input_t *input, click_count_t count, cb_handler fn) {
  ...
  Serial.printf("add_click_handler id - %d   num - %d\n", input->id, input_actions_indices[input->id]);
...
void timer_general_handler(void *arg) {
  ...
  Serial.printf("timer_general_handler id - %d\n\n", index);
...
void register_click(void *arg, bool type) {
  ...
  if (esp_timer_is_active(timer_general_handles[input->id])) {
    esp_timer_delete(timer_general_handles[input->id]);
  }
  esp_timer_start_once(timer_general_handles[input->id], WAIT_FOR_CLICK_MS * 1000);
}
...
void handle_debounce(void *arg) {
  ...
  printf("GPIO state = %d - level = %d - state = %d\n", state, input->level, input->state);
  ...
  else { input->last_click_timestamp = curr_timestamp; }
  input->state = state;
}
...
void handle_no_debounce(void *arg) {
  ...
  esp_timer_start_once(timer_general_handles[input->id], DISABLE_IQR_MS * 1000);
}

void handle_input_interrupt(void *arg) {
  input_t *input = (input_t *)arg;

  Serial.printf("handle_input_interrupt pin - %d id - %d state - %d \n", input->pin, input->id, input->state);
...
void add_dbl_click_handler(input_t *input) {
  click_count_t dbl_click = {2, 0};
  add_click_handler(input, dbl_click, input->on_dbl_click);
}

input_t * create_input_params(gpio_num_t pin, bool level, bool has_debounce, uint32_t debounce_ms, void (*on_click)(input_t *), void (*on_hold)(input_t *), void (*on_dbl_click)(input_t *)) {
...
input_t * create_input(gpio_num_t pin, bool level, bool has_debounce, uint32_t debounce_ms, void (*on_click)(input_t *)) {
  input_t * input = create_input_params(pin, level, has_debounce, debounce_ms, on_click, no_action, no_action);
  return input;
}
...
input_t * create_input(gpio_num_t pin, bool level, bool has_debounce, uint32_t debounce_ms, void (*on_click)(input_t *), void (*on_hold)(input_t *)) {
  input_t * input = create_input_params(pin, level, has_debounce, debounce_ms, on_click, on_hold, no_action);
  add_hold_handler(input);
  return input;
}

input_t * create_input(gpio_num_t pin, bool level, bool has_debounce, uint32_t debounce_ms, void (*on_click)(input_t *), void (*on_hold)(input_t *), void (*on_dbl_click)(input_t *)) {  
  input_t* input = create_input_params(pin, level, has_debounce, debounce_ms, on_click, on_hold, on_dbl_click);
  add_hold_handler(input);
  add_dbl_click_handler(input);
  return input;
}
...
input_t * create_button(gpio_num_t pin, bool level, void (*on_click)(input_t *), void (*on_hold)(input_t *), void (*on_dbl_click)(input_t *)) {  
  return create_input(pin, level, true, DEBOUNCE_MS, on_click, on_hold, on_dbl_click);
}
